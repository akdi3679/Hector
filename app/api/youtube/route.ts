...
export async function GET() {
  ...
  try {
    ...
    for (const c of youtubeChannels) {
      ...
      if (uploads) {
        const pl = await yt(`playlistItems?part=snippet&maxResults=10&playlistId=${uploads}&key=${KEY}`);
    ...
    // Chercher des commentaires dans les 10 dernières vidéos (pas seulement 2)
    let reviews = base.reviews;
    const live: any[] = [];
    
    for (const v of allVideos.slice(0, 10)) {
      if (live.length >= 4) break; // On veut 4 reviews
      try {
        const ct = await yt(`commentThreads?part=snippet&videoId=${v.videoId}&order=relevance&maxResults=3&key=${KEY}`);
        ...
        // Ignorer silencieusement si cette vidéo n'a pas de commentaires
      }
    }
