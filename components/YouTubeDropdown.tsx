...
interface Live { name: string; url: string; handle: string; positioning: string; subscribers: number | null; avatar: string | null; }
...
export default function YouTubeDropdown({ variant = 'ink', drop = 'down' }: { variant?: 'red' | 'ink' | 'ghost'; drop?: 'down' | 'up' }) {
  ...
  const [channels, setChannels] = useState<Live[]>(youtubeChannels.map((c) => ({ ...c, subscribers: null, avatar: null })));
  const ref = useRef<HTMLDivElement>(null);

  useEffect(() => {
    fetch('/api/youtube').then((r) => r.json()).then((d) => d?.channels && setChannels(d.channels)).catch(() => {});
  }, []);
          ...
          {channels.map((c) => (
            <a key={c.handle} href={c.url} target="_blank" rel="noopener noreferrer"
              ...
              {c.avatar
                ...
                : <span className="grid h-9 w-9 place-items-center rounded-full bg-red/10 text-red"><Youtube size={16} /></span>}
              ...
              {c.subscribers != null && <span className="stamp !px-2 !py-1 !text-[9px] !border-sun !text-sun">{fmt(c.subscribers)}</span>}
            </a>
          ))}
        </div>
      )}
